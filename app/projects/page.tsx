...
export default async function ProjectsPage() {
  ...
  if (redis && published.length > 0) {
    try {
      const viewsData = await redis.mget<number[]>(
        ...published.map((p) => ["pageviews", "projects", (p as any).slug].join(":")),
      );
      views = viewsData.reduce((acc, v, i) => {
        acc[(published[i] as any).slug] = v ?? 0;
        return acc;
  ...
  const sorted = published.sort(
    (a, b) =>
      new Date((b as any).date ?? Number.POSITIVE_INFINITY).getTime() -
      new Date((a as any).date ?? Number.POSITIVE_INFINITY).getTime(),
  );
      ...
      <div className="px-6 pt-20 mx-auto space-y-8 max-w-7xl lg:px-8 md:space-y-16 md:pt-24 lg:pt-32">
        ...
        {featured && (
          <div className="grid grid-cols-1 gap-8 mx-auto lg:grid-cols-2 ">
            <Card>
              <Link href={`/projects/${(featured as any).slug}`}>
                <article className="relative w-full h-full p-4 md:p-8">
                  <div className="flex items-center justify-between gap-2">
                    <div className="text-xs text-zinc-100">
                      {(featured as any).date ? (
                        <time dateTime={new Date((featured as any).date).toISOString()}>
                          {Intl.DateTimeFormat(undefined, {
                            dateStyle: "medium",
                          }).format(new Date((featured as any).date))}
                        </time>
                      ) : (
                        <span>SOON</span>
                      )}
